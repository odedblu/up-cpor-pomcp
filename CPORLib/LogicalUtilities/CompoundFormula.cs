    ...
    public class CompoundFormula : Formula
    {
        ...
        */
        ...
        public bool AddOperand(Formula f)
        {
            ...
            else
            {
                ...
                {
                    if (Operator == "and")
                        Operands.Add(new PredicateFormula(Utilities.FALSE_PREDICATE));
                    else if (Operator == "or")
                        Operands.Add(new PredicateFormula(Utilities.TRUE_PREDICATE));
                    else
                        throw new NotImplementedException();
        ...
        public string ToString(int cTabs)
        {
            ...
            for (int i = 0; i < cTabs; i++)
                s += "\t";
            s += "(" + Operator + " " + Utilities.ListToString(Operands, cTabs + 1) + ")";
            return s;
        }
        ...
        public override Formula Reduce(ISet<Predicate> lKnown, bool bContainsNegations)
        {
            ...
            foreach (Formula f in Operands)
            {
                ...
                {
                    if (Operator == "or")
                        return new PredicateFormula(Utilities.TRUE_PREDICATE);//reasoning predicate no longer informative
                ...
                else if (bFalse || !bTrue && fNew.IsFalse(null))
                {
                    if (Operator == "and")
                    {
                        return new PredicateFormula(Utilities.FALSE_PREDICATE);//formula must be false
                    }
                ...
                else
                {
                    ...
                    if (bOperandExists && Operator == "oneof")
                        return new PredicateFormula(Utilities.FALSE_PREDICATE);//oneof requires only one - duplicate means trouble
                }
            ...
            {
                if (Operator == "and")
                    return new PredicateFormula(Utilities.TRUE_PREDICATE);//no negative found so total value is true
                if (Operator == "or")
                    return new PredicateFormula(Utilities.FALSE_PREDICATE);//no positive found so total value is false
            }
                ...
                else
                {
                    ...
                    if (fReduce is PredicateFormula)
                    {
                        if (((PredicateFormula)fReduce).Predicate == Utilities.FALSE_PREDICATE)
                            return fReduce;
                        else if (((PredicateFormula)fReduce).Predicate != Utilities.TRUE_PREDICATE)
            ...
            else if (Operator == "and" || Operator == "or" || Operator == "oneof")
            {
                ...
                foreach (Formula f in Operands)
                {
                    ...
                    else
                    {
                        ...
                        else if (p == Utilities.FALSE_PREDICATE)
                        {
                            if (Operator == "and")
                                return fSimplify;
                        }
        ...
        */
        ...
        public CompoundFormula ToCNF2()
        {
            ...
            {
                bChanged = false;
                cfRevised = cfRevised.RemoveNestedConjunction(out bChanged);
            }
            /*
